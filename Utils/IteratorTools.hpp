...
public:
    ...
    decltype (Utils::move (*mInput)) operator* ()
    {
        return Utils::move(*mInput);
    }
...
public:
    ActionIterator(const InputIterator &iterator, FunctionType &&function)
        : mInput{iterator}, mFunction{Utils::move (function)}
    {
    }
...
{
    return ActionIterator<InputIterator, FunctionType>{inputIterator, Utils::move (function)};
}
