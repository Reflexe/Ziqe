...
public:
    struct Node {
        ...
        Node(Node *parent, Node *right, Node *left, const KeyType &key, Args&&...args)
            : mKeyAndValue{Utils::move (key), {Utils::forward<Args>(args)...}}, mParent{parent}, mRight{right}, mLeft{left}
        {
        }
                                   ...
                                   findAfter, (const KeyType &key),
    {
        ...
        if (findNodeResult.second == false)
            ...
            return {Utils::prev(iterator), iterator};
    })
...
public:
    ...
    Pair<Iterator, bool> insert (const KeyType &key, Args&&... args) {
        ...
        auto iteratorAndIsSucceed = BinaryTreeType::insert (key, Utils::forward<Args>(args)...);
