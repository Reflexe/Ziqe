...
#define ZQ_DEFINE_TRIVIAL_HASH(type) \
template<>\
struct Hash<type>\
{\
...
ZQ_DEFINE_TRIVIAL_HASH(uint8_t)
ZQ_DEFINE_TRIVIAL_HASH(int8_t)
ZQ_DEFINE_TRIVIAL_HASH(uint16_t)
ZQ_DEFINE_TRIVIAL_HASH(int16_t)
ZQ_DEFINE_TRIVIAL_HASH(uint32_t)
ZQ_DEFINE_TRIVIAL_HASH(int32_t)
ZQ_DEFINE_TRIVIAL_HASH(uint64_t)
ZQ_DEFINE_TRIVIAL_HASH(int64_t)

#undef ZQ_DEFINE_TRIVIAL_HASH
 ...
 *
 * The design of this hash table is simple: the table (mTable) is a vector of iterators
    ...
    struct PairType : RealPairType {
        ...
        explicit PairType(const KeyType &key,
                          const typename TableType::Iterator &parameterTableIterator,
                           Args&&... args)
            : RealPairType{key, T{Utils::forward<Args>(args)...}},
               ...
               ++iterator)
          {
              ...
              if (! isAFirstEntryRemoved) {
                  ...
                  if (tableIterator != firstEntryTableIterator) {
                      ...
                      // tableIterator to).
                      *firstEntryTableIterator = this->end ();
...
public:
   ...
   /**
    ...
    Iterator insertOrAssign(const KeyType &key,
                            Args&&... args)
    {
        auto result = insert (key, Utils::forward<Args>(args)...);

        if (! result.first)
            result.second->second = T{Utils::forward<Args>(args)...};
    ...
    Iterator insertOrAssign(const KeyType &key,
                            const T& args)
    {
        auto result = insert (key, Utils::forward<T&>(args));
    ...
    Iterator insertOrAssign(const KeyType &key,
                            T&& args)
    {
        auto result = insert (key, Utils::move (args));

        if (! result.first)
            result.second->second = Utils::move (args);
