...
#include "Utils/ExtendedVector.hpp"
...
public:
    ...
    FieldReader(_VectorReferenceType &&vector)
        : mVector{Utils::forward<_VectorReferenceType&&> (vector)}
    {
    }
    ...
    template<class T>
    Utils::Expected<T, ReadError> tryReadT () {
        if (! canReadT<T> ())
    ...
    template<class T>
    Utils::Expected<Utils::Vector<T>, ReadError> tryReadTVector (SizeType numberOfElements) {
