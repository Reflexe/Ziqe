...
#define ZQ_ALLOW_EXPECTED() template<class A, class B> friend struct ::Ziqe::Base::CustomStorageConstructor
 ...
 */
template<class... Args>
constexpr bool IgnoreUnused (Args&&...)
{
    return true;
}
...
template<bool value, class T=void>
using EnableIf=typename _EnableIf<value, T>::type;
...
{
    return Triple<T1, T2, T3>{forward<T1>(first), forward<T2>(second), forward<T3>(third)};
}

template<typename F, typename... Args>
void pack_foreach(F f, Args&&... args) {
    int unpack[] = {(f(forward<Args>(args)), 0)..., 0};
...
{
    return arg + plusArgs<ResultType>(forward<Args>(args)...);
}
...
template<SizeType...Ints>
using IndexSequence=IntegerSequence<SizeType, Ints...>;

namespace Internal {

template<class T, SizeType N, T ...PreviousNumbers>
struct GenerateSequence
{
    typedef typename GenerateSequence<T, N-1, N-1, PreviousNumbers...>::type type;
};

template<class T, T ...PreviousNumbers>
struct GenerateSequence<T, 0, PreviousNumbers...>
{
    typedef IntegerSequence<T, PreviousNumbers...> type;
};

} // namespace Internal

template<class T, T N>
using MakeIntegerSequence=typename Internal::GenerateSequence<T, N>::type;

template<SizeType N>
using MakeIndexSequence=MakeIntegerSequence<SizeType, N>;
